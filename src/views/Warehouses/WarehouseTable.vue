        ...
        <div class="card-header border-0" :class="type === 'dark' ? 'bg-transparent': ''">
            <div class="row">
                ...
                <div class="col text-right">
                    <base-button type="primary" @click="$emit('add')">Add Warehouse</base-button>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        view(index) {
            this.$router.push({ path: `warehouses/${index}` })
        },
        deleteItem(id) {
            let item = this.warehouses.find(el => el.id == id)
    ...
    computed: {
        warehouses() {
            return this.$store.state.warehouses.items
        },
        pagination() {
            return this.$store.state.warehouses.pagination
        },
    },
}
</script>
